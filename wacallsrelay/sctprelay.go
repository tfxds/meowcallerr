...
// wireLog dumps every datachannel packet (TX/RX) to stderr in hex with a
// millisecond timestamp, size, and a deduced packet type. Hex is truncated to
...
func wireLog(dir, id string, data []byte) {
	...
	n := len(data)
	cls := ClassifyPacket(data)
	showLen := n
	suffix := ""
	if n > 200 {
		showLen = 64
		suffix = fmt.Sprintf("...(+%dB)", n-64)
	}
	hx := hex.EncodeToString(data[:showLen]) + suffix
	fmt.Fprintf(os.Stderr, "[%s] %s id=%s len=%d type=%q hex=%s\n", ts, dir, id, n, cls, hx)
}
...
func (m *SctpRelayManager) connectToRelay(info RelayConfig) {
	...
	pc.OnICEConnectionStateChange(func(s webrtc.ICEConnectionState) {
		fmt.Fprintf(os.Stderr, "[%s] [WIRE-ICE] id=%s state=%s\n", time.Now().Format("15:04:05.000"), id, s.String())
	...
	channel.OnOpen(func() {
		fmt.Fprintf(os.Stderr, "[%s] [WIRE-OPEN] id=%s datachannel open\n", time.Now().Format("15:04:05.000"), id)
...
func (m *SctpRelayManager) modifySdpForRelay(sdp string, info RelayConfig) string {
	...
	out = reFingerprint.ReplaceAllString(out, "a=fingerprint:"+WADTLSFingerprint)
