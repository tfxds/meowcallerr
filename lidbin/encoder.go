...
var errTipoInvalido = errors.New("lidbin: tipo invalido no conteudo do no")
...
func (w *binaryEncoder) write(data any) {
	...
	default:
		panic(fmt.Errorf("%w: %T", errTipoInvalido, typedData))
	}
}
...
func (w *binaryEncoder) writeJID(jid types.JID) {
	// ⭐ DIFERENÇA DELIBERADA em relação ao whatsmeow: um JID de LID com device 0 aqui sai
	// como ADJID (agente 1), não como JIDPair. É o que o Baileys emite (`:0@lid` tem device
	// explícito) e é a única forma que o motor whatsapp.wasm reconhece como domínio @lid —
	// com JIDPair ele lê o creator como @s.whatsapp.net e descarta a chamada
	// ("mismatched peer id and creator id").
...
func (w *binaryEncoder) countAttributes(attributes waBinary.Attrs) (count int) {
...
// MarshalParaWasm serializa o nó no formato que o motor whatsapp.wasm consome.
// `comFlags=false` remove o byte de flags do começo (o Marshal do whatsmeow inclui; o
// encodeBinaryNode do Baileys, que é o que o SheIITear entrega ao motor, não inclui).
func MarshalParaWasm(n waBinary.Node, comFlags bool) []byte {
	w := newEncoder()
	w.writeNode(n)
	d := w.getData()
	if !comFlags && len(d) > 0 {
